...
def cull_cells_fixed_death_rate(x, y, z, cell_types, cell_driv_muts, positions, deme_pops, deme_size, s, death_rate):
    pos, pop = positions[x][y][z], deme_pops[x][y][z]
    #find properties
    if pop > deme_size:
        types_here, driv_muts_here= cell_types[pos][:pop], cell_driv_muts[pos][:pop]
        survival_probs = 1- death_rate*np.power(1-s, driv_muts_here)
...
def initial_growth_expansion(cell_types, cell_driv_muts, deme_pops, subclone_parentage, muts_per_clone, drivs_per_clone, num_types, positions, necrotic_marker, ur_nos, ur_tracker, s, death_rate, division_prob, driv_prob, exp_muts, arrlength, spno, mno, dno, expno, deme_size, scratch, mut_rand_nos, mut_tracker, xs, ys, zs, scratch_limit, no_demes_total=10000, nr=1000000000):
    ...
    for n in order:
        ...
        if len(empty_neighbours) == 0:
            ...
            if deme_pops[x][y][z] > 0.1*deme_size: #allow the first 1000 cells to grow without any death rate, to prevent tumour dying off
              cell_types, cell_driv_muts, deme_pops = cull_cells_fixed_death_rate(x, y, z, cell_types, cell_driv_muts, positions, deme_pops, deme_size, s, death_rate)
...
def run_to_full_size(expno, scratch, s, death_rate, division_prob, driv_prob, exp_muts, no_demes_total, no_cells_total, gridlength, deme_size, scratch_limit, nr=100000000, arrlength=1000000):
    ...
    while total_cells < no_cells_total:
        cell_types, cell_driv_muts, deme_pops, num_types, positions, necrotic_marker, ur_nos, ur_tracker, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, xs, ys, zs = initial_growth_expansion(cell_types, cell_driv_muts, deme_pops, subclone_parentage, muts_per_clone, drivs_per_clone, num_types, positions, necrotic_marker, ur_nos, ur_tracker, s, death_rate, division_prob, driv_prob, exp_muts, arrlength, spno, mno, dno, expno, deme_size, scratch, mut_rand_nos, mut_tracker, xs, ys, zs, scratch_limit, no_demes_total=no_demes_total, nr=nr)
    #find occupied deme
...
def main():
    ...
    parser.add_argument('--death_rate', type=float, required=True, help='Death rate of cells.')
